...
generator = mlrose_hiive.KnapsackGenerator()
problems = []
for i in range(30): # GA DID BEST
    problem = generator.generate(seed=i, number_of_items_types=20, max_item_count=10, max_weight_per_item=10, max_value_per_item=10, max_weight_pct=.6)
...
for noise in [0, .05]:
    for keep_pct in [.2, .5]:
        ...
        for problem in problems:
            start = time.time()
            MIMIC_best_state, MIMIC_best_fit, MIMIC_fits = mlrose_hiive.mimic(problem, pop_size=25, keep_pct=keep_pct, noise=noise, max_attempts=10, max_iters=500,  curve=True, random_state=1)
            end = time.time()
...
plt.title("Knapsack Fitness vs Iterations")
plt.xlabel("Iterations")
plt.ylabel("Fitness")
plt.legend()
plt.savefig("knapsack_plt.png")

for ga_run in ga_scores:
    pop_breed_percent, mutation_prob, ga_score, ga_length, ga_vals, ga_time = ga_run
    ga_pads = [np.pad(x, (0, 500 - len(x)), 'constant', constant_values=x[-1]) for x in ga_vals]
    ga_plot = np.mean(np.vstack(ga_pads), axis=0)

    plt.plot(ga_plot, label="Breed " + str(pop_breed_percent) + " Mutation " + str(mutation_prob))
    plt.title("Genetic Algorithm Params vs Iterations")
    plt.xlabel("Iterations")
    plt.ylabel("Fitness")
    plt.legend()
    plt.savefig('ga_hpt.png')
